...
{
    public partial class frmNacinDolaskaOdlaska : Form
    {


        string KorisnikCene;
        bool status = false;
        public frmNacinDolaskaOdlaska()
        {
            InitializeComponent();
        }

        public frmNacinDolaskaOdlaska(string Korisnik)
        {
            InitializeComponent();
        ...
        private void tsSave_Click(object sender, EventArgs e)
        {
            if (status == true)
            {
                ...
                ins.InsNacinDolaskaOdlaska(txtNaziv.Text, Convert.ToDateTime(DateTime.Now), KorisnikCene);
                status = false;
            }
            else
            {
        ...
        private void tsDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                InsertNacinDolaskaOdlaska upd = new InsertNacinDolaskaOdlaska();
                upd.DeleteNacinDolaskaOdlaska(Convert.ToInt32(txtSifra.Text));
                RefreshDataGrid();
            }
        ...
        {
            var select = " SELECT [ID] ,Naziv,[Datum],[Korisnik] FROM [dbo].[NacinDolaskaOdlaska]";
        ...
        private void VratiPodatke(string ID)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT ID , Naziv from NacinDolaskaOdlaska where ID=" + txtSifra.Text, con);
        ...
        private void tsPrvi_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("select Min([ID]) as ID from NacinDolaskaOdlaska", con);
            SqlDataReader dr = cmd.ExecuteReader();
        ...
        private void tsPoslednja_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("select Max([ID]) as ID from NacinDolaskaOdlaska", con);
            SqlDataReader dr = cmd.ExecuteReader();
        ...
        private void tsNazad_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("select top 1 ID as ID from NacinDolaskaOdlaska where ID <" + Convert.ToInt32(txtSifra.Text) + " Order by ID desc", con);
            SqlDataReader dr = cmd.ExecuteReader();
        ...
        private void tsNapred_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("select top 1 ID as ID from NacinDolaskaOdlaska where ID >" + Convert.ToInt32(txtSifra.Text) + " Order by ID", con);
