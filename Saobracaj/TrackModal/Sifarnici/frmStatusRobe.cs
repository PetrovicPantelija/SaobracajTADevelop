...
{
    public partial class frmStatusRobe : Form
    {


        string KorisnikCene;
        bool status = false;
        public frmStatusRobe()
        {
            InitializeComponent();
        }

        public frmStatusRobe(string Korisnik)
        {
            InitializeComponent();
        ...
        private void tsSave_Click(object sender, EventArgs e)
        {
            if (status == true)
            {
                InsertStatusRobe ins = new InsertStatusRobe();
                ins.InsStatusRobe(txtNaziv.Text, Convert.ToDateTime(DateTime.Now), KorisnikCene);
                status = false;
            }
            else
            {
                //int TipCenovnika ,int Komitent, double Cena , int VrstaManipulacije ,DateTime  Datum , string Korisnik
                InsertStatusRobe upd = new InsertStatusRobe();
                upd.UpdStatusRobe(Convert.ToInt32(txtSifra.Text), txtNaziv.Text, Convert.ToDateTime(DateTime.Now), KorisnikCene);
            }
            RefreshDataGrid();
        }
        ...
        {
            var select = " SELECT [ID] ,Naziv,[Datum],[Korisnik] FROM [dbo].[StatusRobe]";
        ...
        private void VratiPodatke(string ID)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT ID , Naziv from StatusRobe where ID=" + txtSifra.Text, con);
        ...
        private void tsPrvi_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("select Min([ID]) as ID from StatusRobe", con);
            SqlDataReader dr = cmd.ExecuteReader();
        ...
        private void tsPoslednja_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("select Max([ID]) as ID from StatusRobe", con);
            SqlDataReader dr = cmd.ExecuteReader();
        ...
        private void tsNazad_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("select top 1 ID as ID from StatusRobe where ID <" + Convert.ToInt32(txtSifra.Text) + " Order by ID desc", con);
            SqlDataReader dr = cmd.ExecuteReader();
        ...
        private void tsNapred_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("select top 1 ID as ID from StatusRobe where ID >" + Convert.ToInt32(txtSifra.Text) + " Order by ID", con);
