...
namespace Testiranje.Sifarnici
{
    public partial class frmGreske : Form
    {
        ...
        public frmGreske()
        {
            ...
            public frmGreske(string Korisnik)
        {
            InitializeComponent();
            ...
            private void tsSave_Click(object sender, EventArgs e)
            {
                if (status == true)
                {
                    InsertGreske ins = new InsertGreske();
                    ins.InsGreske(txtNaziv.Text, Convert.ToDateTime(DateTime.Now), KorisnikCene);
                    status = false;
                }
                else
                {
                    //int TipCenovnika ,int Komitent, double Cena , int VrstaManipulacije ,DateTime  Datum , string Korisnik
                    InsertGreske upd = new InsertGreske();
                    upd.UpdGreske(Convert.ToInt32(txtSifra.Text), txtNaziv.Text, Convert.ToDateTime(DateTime.Now), KorisnikCene);
                }
                RefreshDataGrid();
            }
            ...
            {
                InsertGreske upd = new InsertGreske();
                upd.DeleteGreske(Convert.ToInt32(txtSifra.Text));
                RefreshDataGrid();
            }
            ...
            {
                var select = " SELECT [ID] ,Naziv,[Datum],[Korisnik] FROM [dbo].[Greske]";
            ...
            private void VratiPodatke(string ID)
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT ID , Naziv from Greske where ID=" + txtSifra.Text, con);
