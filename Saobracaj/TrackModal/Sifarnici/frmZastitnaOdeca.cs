...
{
    public partial class frmZastitnaOdeca : Form
    {
        string KorisnikCene;
        bool status = false;
        public frmZastitnaOdeca()
        {
            InitializeComponent();
        }

        public frmZastitnaOdeca(string Korisnik)
        {
            InitializeComponent();
        ...
        private void tsSave_Click(object sender, EventArgs e)
        {
            if (status == true)
            {
                InsertZastitnaOdeca ins = new InsertZastitnaOdeca();
                ins.InsZastitna(txtNaziv.Text, Convert.ToDateTime(DateTime.Now), KorisnikCene);
                status = false;
            }
            else
            {
                //int TipCenovnika ,int Komitent, double Cena , int VrstaManipulacije ,DateTime  Datum , string Korisnik
                InsertZastitnaOdeca upd = new InsertZastitnaOdeca();
                upd.UpdZastitna(Convert.ToInt32(txtSifra.Text), txtNaziv.Text, Convert.ToDateTime(DateTime.Now), KorisnikCene);
            }
            RefreshDataGrid();
        }
        ...
        private void tsDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                InsertZastitnaOdeca upd = new InsertZastitnaOdeca();
                upd.DeleteZastitna(Convert.ToInt32(txtSifra.Text));
                RefreshDataGrid();
            }
        ...
        {
            var select = " SELECT [ID] ,Naziv,[Datum],[Korisnik] FROM [dbo].[ZastitnaOdeca]";
        ...
        private void VratiPodatke(string ID)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT ID , Naziv from ZastitnaOdeca where ID=" + txtSifra.Text, con);
        ...
        private void frmZastitnaOdeca_Load(object sender, EventArgs e)
        {
            RefreshDataGrid();
