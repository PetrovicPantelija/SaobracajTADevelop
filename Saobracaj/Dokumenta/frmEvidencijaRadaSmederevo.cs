...
{
    public partial class frmEvidencijaRadaSmederevo : Form
    {
        MailMessage mailMessage;
        Boolean status = false;

        public frmEvidencijaRadaSmederevo()
        {
            InitializeComponent();
        }

        public frmEvidencijaRadaSmederevo(int Sifra)
        {
            InitializeComponent();
        ...
        private void frmEvidencijaRada_Load(object sender, EventArgs e)
        {
            ...
            var select4 = " select ID, Naziv from VrstaAktivnosti where ObracunPoSatu = 1 and Smederevo = 1";
        ...
        private void tsSave_Click(object sender, EventArgs e)
        {
            ...
            if (status == true)
            {
                ...
                ins.InsAktivnosti(Convert.ToInt32(cboZaposleni.SelectedValue), dtpVremeOd.Value, dtpVremeDo.Value, vreme, Convert.ToDouble(txtTrosak.Text), txtKomentar.Text, 0, txtOznaka.Text, Convert.ToDouble(txtRacun.Text), Convert.ToDouble(txtKartica.Text), masinovodja, txtMesto.Text.Trim(),0,1,55,1, "sa", 0);
                status = false;
                VratiPodatkeMax();
            }
            else
            {
                ...
                upd.UpdAktivnosti(Convert.ToInt32(txtSifra.Text), Convert.ToInt32(cboZaposleni.SelectedValue), dtpVremeOd.Value, dtpVremeDo.Value, vreme, Convert.ToDouble(txtTrosak.Text), txtKomentar.Text, 0, txtOznaka.Text, Convert.ToDouble(txtRacun.Text), Convert.ToDouble(txtKartica.Text), masinovodja, txtMesto.Text.Trim(),0,1,55);
            }
        }

        private void RefreshDataGrid()
        {
            var select = "  select ID, Naziv from VrstaAktivnosti where ObracunPoSatu=1 and Smederevo = 1";
